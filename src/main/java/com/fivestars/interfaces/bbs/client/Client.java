...
import com.fivestars.interfaces.bbs.api.UC;
...
public class Client extends PHPFunctions{
    /**
     * 向Client类提供HttpServletRequest的提供者
     * @author Amyson Wu
     *
     */
    public static interface HSRProvider{
        public HttpServletRequest getRequest();
    }

    private static HSRProvider hsrProvider = null;
    public static void addHSRProvider(HSRProvider hsrp) {
        hsrProvider = hsrp;
    }
    ...
    static {
        InputStream in = null;
        try {
            in = Client.class.getClassLoader().getResourceAsStream("dz-client4j.properties");
            init(in);
        } catch (Exception e) {
        } finally {
            if(in != null) {
    ...
    public static void init(InputStream in) throws Exception{
        ...
        if(!UC_API.toString().startsWith("http")) {
            if(!UC_API.startsWith("/")) UC_API = "/" + UC_API;
        }
        ...
        if("1".equals(enabled)) UC.enabled = true;
    }

    public static HttpServletRequest getRequest() {
        HttpServletRequest req = UC.getRequest();
        if(req == null && hsrProvider != null)
            req = hsrProvider.getRequest();
        return req;
    }
...
//		}
    ...
    public String uc_api_post(String $module, String $action, Map<String,Object> $arg ) {
        ...
        return uc_fopen2(getUCApi()+"/index.php", 500000, $postdata, "", true, UC_IP, 20, true);
    }
     ...
     * @param string $extras		附加参数（传送时不加密）
     * @return string
     */
    protected String uc_api_requestdata(String $module, String $action, String $arg, String $extras) {
        String $input = uc_api_input($arg);
        String $post = "m="+$module+"&a="+$action+"&inajax=2&release="+UC_CLIENT_RELEASE+"&input="+$input+"&appid="+UC_APPID+$extras;
        return $post;
    }

    protected String uc_api_url(String $module, String $action, String $arg, String $extras) {
        String $url = getUCApi()+"/index.php?"+uc_api_requestdata($module, $action, $arg, $extras);
        return $url;
    }
    ...
    public String uc_api_input(String $data) {
        ...
        if(hsr != null) agent = hsr.getHeader("HTTP_USER_AGENT");
        if(agent == null) agent = "";
        String $s = urlencode(uc_authcode($data+"&agent="+md5(agent)+"&time="+time(), "ENCODE", UC_KEY));
        return $s;
    }
...
//		}
    ...
    protected String uc_fopen(String $url, int $limit, String $post, String $cookie, boolean $bysocket ,String $ip , int $timeout , boolean $block ) {
        ...
        try {
            ...
            if( $matches.getPort()>0 ) $port = getRequest().getServerPort();
        ...
        try{
            ...
            System.out.print("");/* do nothing,just to debug */
        }
        return $return;
    }
    ...
    public String uc_user_login(String $username, String $password){
        return uc_user_login($username, $password, 0, 0, null);
    }
    ...
    -2 : 密码错
     */
    public String uc_user_login(String $username, String $password, Map<String,String> extras){
        return uc_user_login($username, $password, 0, 0, extras);
    }
    public String uc_user_login(String $username, String $password, int $isuid , int $checkques, Map<String,String> extras ){
        return uc_user_login($username, $password, $isuid, $checkques, "","", extras);
    }
    public String uc_user_login(String $username, String $password, int $isuid , int $checkques , String $questionid, String $answer, Map<String,String> extras) {
        Map<String,Object> args = new HashMap<String, Object>();
        args.put("username",$username);
        args.put("password",$password);
